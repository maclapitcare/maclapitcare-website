...
import { CheckCircle, Star, ArrowRight, Laptop, Phone, MessageCircle, Clock, Shield, Wrench, HardDrive, Cpu, Zap, Monitor } from "lucide-react";
...
export default function LaptopRepair() {
  ...
  const laptopBrands = [
    {
      ...
      commonIssues: ["Screen flickering", "Overheating issues", "Battery not charging", "Broken hinges"],
      solutions: ["Display cable repair", "Thermal paste replacement", "Charging port fix", "Hinge repair & replacement"],
      searchKeywords: ["Dell laptop repair Delhi", "Inspiron screen replacement", "Dell overheating fix"],
      link: "/dell-laptop-repair-noida"
    },
    {
      name: "HP Laptops", 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-slate-50">
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12 md:mb-16">
            <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-800 px-3 md:px-4 py-2 rounded-full text-xs md:text-sm font-semibold mb-4 md:mb-6">
              ...
              Brand-Wise Laptop Repair in Noida
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 md:mb-6">
              <span className="text-blue-600">Expert Laptop Repair Services</span> by Brand
            </h2>
            <p className="text-lg md:text-xl text-gray-600 max-w-4xl mx-auto leading-relaxed px-4">
              Professional laptop repair services in Noida Delhi for all major brands with genuine parts and warranty
            </p>
          </div>
            ...
            {[
              {
                ...
                icon: Laptop,
                features: ["Screen replacement", "Overheating solutions", "Motherboard repair", "Data recovery"],
                specialties: "Inspiron, XPS, Latitude, Alienware, Vostro, Precision"
              },
              {
                ...
                icon: Monitor,
                features: ["Keyboard replacement", "BIOS recovery", "Charging port repair", "WiFi module fix"],
                specialties: "Pavilion, Envy, EliteBook, ProBook, Omen, Spectre"
              },
              {
                ...
                icon: Zap,
                features: ["Gaming performance", "Graphics card repair", "Thermal management", "RGB lighting fix"],
                specialties: "ZenBook, VivoBook, ROG, TUF Gaming, ExpertBook"
              },
              {
                ...
                icon: Wrench,
                features: ["Hinge replacement", "Trackpad calibration", "Performance upgrade", "Cooling system"],
                specialties: "ThinkPad, IdeaPad, Legion, Yoga, ThinkBook"
              },
              {
                ...
                icon: Cpu,
                features: ["Gaming optimization", "Advanced cooling", "RGB controller repair", "Performance tuning"],
                specialties: "Gaming Series, Creator, Modern, Prestige, Stealth"
              },
              {
                ...
                icon: HardDrive,
                features: ["SSD installation", "RAM upgrade", "Performance boost", "System optimization"],
                specialties: "Aspire, Predator, Swift, Spin, Nitro, ConceptD"
              }
            ].map((brand, index) => (
              <Card key={index} className="group hover:shadow-xl transition-all duration-300 bg-white border-2 hover:border-blue-200 rounded-xl">
                <CardHeader className="text-center pb-3">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl flex items-center justify-center mx-auto mb-4">
                    <brand.icon className="h-6 w-6 md:h-8 md:w-8 text-white" />
                  </div>
                  <CardTitle className="text-lg md:text-xl text-blue-700 font-bold">{brand.name}</CardTitle>
                </CardHeader>
                <CardContent className="text-center px-6">
                  <p className="text-gray-600 text-sm mb-4 leading-relaxed">{brand.description}</p>
                  
                  <div className="mb-4">
                    <p className="text-xs font-semibold text-blue-600 mb-2">MODELS COVERED:</p>
                    <p className="text-xs text-gray-600 leading-relaxed">{brand.specialties}</p>
                  </div>
                  ...
                  <div className="flex items-center justify-center space-x-2 text-sm bg-green-50 p-2 rounded-lg">
          ...
          <div className="text-center bg-white rounded-xl p-8 shadow-lg">
            ...
            <p className="text-gray-600 mb-6 text-lg">
              Our certified technicians specialize in all major laptop brands with genuine parts and comprehensive warranty
            </p>
            <div className="flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-6">
              <a
                href={`tel:+91${CONTACT_INFO.phone1}`}
                className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg font-semibold transition-colors flex items-center space-x-2 text-lg"
              >
      ...
      {/* Original Laptop Brands Section */}
      <section className="py-12 md:py-20 bg-gradient-to-br from-gray-50 to-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12 md:mb-16">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 md:mb-6">
              <span className="text-blue-600">Complete</span> Laptop Model Coverage
            </h2>
            <p className="text-lg md:text-xl text-gray-600 max-w-4xl mx-auto leading-relaxed px-4">
              Comprehensive repair services for all laptop models with detailed solutions for common issues
            </p>
          </div>
              ...
              <div className="flex gap-4 md:gap-6 min-w-max px-2 md:px-4">
                {laptopBrands.map((brand, index) => {
                  const CardComponent = ({ children }: { children: React.ReactNode }) => {
                    if (brand.link) {
                      return (
                        <Link href={brand.link}>
                          <Card className="group hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 bg-white/80 backdrop-blur-sm border-0 rounded-xl md:rounded-2xl overflow-hidden flex-shrink-0 w-[320px] md:w-[400px] cursor-pointer hover:border-blue-300">
                            {children}
                          </Card>
                        </Link>
                      );
                    ...
                    return (
                      <Card className="group hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 bg-white/80 backdrop-blur-sm border-0 rounded-xl md:rounded-2xl overflow-hidden flex-shrink-0 w-[320px] md:w-[400px]">
                        {children}
                      </Card>
                    );
                  };
                      ...
                      <CardContent className="p-4 md:p-8 h-full">
                        <div className="flex items-start justify-between mb-4 md:mb-6">
                          ...
                          <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-blue-600 to-blue-700 rounded-lg md:rounded-xl flex items-center justify-center flex-shrink-0 ml-2">
                            ...
                            {brand.link && (
                              <ArrowRight className="absolute top-2 right-2 h-3 w-3 text-white opacity-0 group-hover:opacity-100 transition-opacity" />
                            )}
                          </div>
                        </div>
                          ...
                          {brand.link && (
                            <div className="mt-2 md:mt-3">
                              <div className="text-xs text-blue-600 font-semibold flex items-center gap-1">
                                Click for dedicated page <ArrowRight className="h-3 w-3" />
                              </div>
                            </div>
                          )}
                        </div>
